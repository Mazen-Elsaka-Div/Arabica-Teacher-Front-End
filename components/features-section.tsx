  ...
  return (
    <section id="features" className="relative bg-background py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        <div ref={listRef} className="mt-14 border-t border-border">
          {features.map((f) => {
            const Icon = f.icon
            return (
              <div
                key={f.step}
                className="feature-row group grid grid-cols-[auto_1fr] items-start gap-5 border-b border-border py-8 transition-colors hover:bg-secondary/40 md:grid-cols-[6rem_3rem_1fr] md:items-center md:gap-8 md:px-4"
              >
                <span className="text-3xl font-black text-foreground/15 transition-colors group-hover:text-gold md:text-5xl">
                  {f.step}
                </span>

                <span className="row-start-1 grid size-12 place-items-center rounded-xl bg-primary text-primary-foreground transition-transform duration-300 group-hover:-translate-y-1 md:row-auto">
