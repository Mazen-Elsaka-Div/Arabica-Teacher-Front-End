...
/* ─────────────────────────────────────────────
 * SiteLoader — splash screen that:
 * 1. Preloads every heavy image (both light & dark topo
 *    backgrounds so theme switching is instant later)
 * 2. Stays visible for a minimum of 1 second
 * 3. Fades out smoothly once everything is ready
 * ───────────────────────────────────────────── */
...
const MIN_DURATION = 1000 // ms — الحد الأدنى لعرض شاشة التحميل

function preload(src: string) {
  return new Promise<void>((resolve) => {
    const img = new Image()
    img.onload = () => resolve()
    img.onerror = () => resolve() // لا نعطّل الموقع لو صورة فشلت
    img.src = src
  })
}

export function SiteLoader() {
  const [hidden, setHidden] = useState(false) // starts visible
  ...
  useEffect(() => {
    ...
    Promise.all(PRELOAD_IMAGES.map(preload)).then(() => {
      ...
      setTimeout(() => {
        if (cancelled) return
        setLeaving(true)
        // بعد انتهاء الـ fade نشيل العنصر من الشجرة تمامًا
